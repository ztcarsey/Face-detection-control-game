...
pygame.display.set_caption('人脸游戏')
...
def main():
    ...
    #游戏正式开始
    cap,detector = cv2.VideoCapture(0),FaceDetector()
    running = True
    while running:
        success, img = cap.read()
        img=cv2.flip(img,3)
        img, bboxs = detector.findFaces(img)
        #print(bboxs[0][1][0])# 脸部在X轴上的位置
        ...
        elif level == 3 and your_score > 30000:
            level = 4
            #增加5加小型敌机，3架中型敌机，2加大型敌机
            add_small_planes(small_planes, enemies, 5)
            add_middle_planes(middle_planes, enemies, 3)
        ...
        elif level == 4 and your_score > 100000:
            level = 5
            #增加5加小型敌机，3架中型敌机，2加大型敌机
            add_small_planes(small_planes, enemies, 5)
            add_middle_planes(middle_planes, enemies, 3)
        ...
        try:
            me.move(bg_size[0],img.shape[0],bboxs[0][1][0])#人脸控制左右
        except:
            pass
